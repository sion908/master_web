...
# Sion908 Blog ローカルサーバー起動スクリプト
# 使い方: ./serve.sh [-p PORT] [-b] [-s] [-d]
...
start_dev_server() {
    ...
    # ファイル監視付きサーバー起動
    watchexec --exts rst,py,css,js --make-project-nothing --restart make html &
    WATCH_PID=$!
